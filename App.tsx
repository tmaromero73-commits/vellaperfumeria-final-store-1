...
// --- COMPONENTES AISLADOS (Para evitar errores de corte) ---
...
const App: React.FC = () => {
    ...
    const addToCartLogic = (product: Product, selectedVariant: Record<string, string> | null) => {
    ...
    const handleAddToCart = (product: Product, buttonElement: HTMLButtonElement | null, selectedVariant: Record<string, string> | null) => {
        addToCartLogic(product, selectedVariant);
        setIsCartOpen(true);
    };
    ...
    const buildBreadcrumbs = (): BreadcrumbItem[] => {
        ...
        switch(view.current) {
            ...
            case 'productDetail':
                ...
                break;
            case 'ofertas': crumbs.push({ label: 'Ideas Regalo' }); break;
            case 'ia': crumbs.push({ label: 'Asistente IA' }); break;
            case 'catalog': crumbs.push({ label: 'Catálogo' }); break;
            case 'blog': crumbs.push({ label: 'Blog' }); break;
            case 'blogPost':
                crumbs.push({ label: 'Blog', onClick: () => handleNavigate('blog') });
                crumbs.push({ label: view.payload.title });
                break;
            case 'checkoutSummary': crumbs.push({ label: 'Finalizar Compra' }); break;
        }
        return crumbs;
    };
    ...
    const renderContent = () => {
        ...
        switch (view.current) {
            case 'home': return <ProductList {...commonProps} />;
            case 'products': return <ShopPage initialCategory={view.payload || 'all'} {...commonProps} />;
            case 'productDetail': return <ProductDetailPage product={view.payload} {...commonProps} />;
            case 'ofertas': return <OfertasPage {...commonProps} />;
            case 'ia': return <AsistenteIAPage cartItems={cartItems} />;
            case 'catalog': return <CatalogPage {...commonProps} />;
            case 'blog': return <BlogPage posts={blogPosts} onSelectPost={(post) => handleNavigate('blogPost', post)} />;
            case 'blogPost': return <BlogPostPage post={view.payload} allPosts={blogPosts} onSelectPost={(post) => handleNavigate('blogPost', post)} onBack={() => handleNavigate('blog')} />;
            case 'checkoutSummary': return <CheckoutSummaryPage cartItems={cartItems} currency={currency} onUpdateQuantity={handleUpdateQuantity} onRemoveItem={handleRemoveItem} onNavigate={handleNavigate} />;
            default: return <ProductList {...commonProps} />;
        }
    };

    return (
        <div className="flex flex-col min-h-screen bg-white font-sans text-gray-800 relative">
            {isLoadingCart && <LoadingBar />}
            ...
            <style>{`
                ...
                ::selection { background-color: var(--color-primary-solid); color: white; }
                .btn-primary {
                ...
                .btn-primary:hover {
                 ...
                 .hover-underline-effect { display: inline-block; position: relative; }
                 ...
                 .hover-underline-effect:hover::after { transform: scaleX(1); transform-origin: bottom left; }
                 @keyframes pop {
                    0% { transform: scale(1); }
